...
- **Core Hypothesis**：在加密、访问控制、主动删除和备份清理门禁全部满足的前提下，长期私密档案与持续补充能力能降低举报准备的整理成本，同时保留用户自主决定权。
- **Data Requirements**：黄金案例集、模型越界率、危机召回率、来源引用正确率、可用性测试，以及长期私密档案的删除/导出/并发控制证据；当前均无生产基线。
