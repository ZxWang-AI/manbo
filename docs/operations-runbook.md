...
- AI 只能使用状态为 `parsed` 且 `eligibleForAi=true` 的、具有来源关联的派生内容。`quarantined`、`scanning`、`scan_failed`、`saved_unread`、`parse_queued` 和 `blocked_malicious` 中的原始材料都必须保留在 AI 输入之外。
