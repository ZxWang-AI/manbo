...
export interface MaterialUploadCancelDeleteHandlerOptions {
  accounts: Pick<AccountRepository, "resumeSession">;
  cases: Pick<CaseRepository, "getPrivate">;
  uploads: Pick<MaterialUploadService, "cancel">;
  isPersistenceAvailable: boolean;
  isObjectStorageAvailable: boolean;
  requestId?: () => string;
}

export function createMaterialUploadCancelDeleteHandler({
  accounts,
  cases,
  uploads,
...
}: MaterialUploadCancelDeleteHandlerOptions) {
  return async function DELETE(
    ...
    try {
      ...
      await uploads.cancel({ accountId: owner.accountId, caseId, uploadId });
...
const defaultHandlers = createMaterialUploadCancelDeleteHandler({
...
export const DELETE = defaultHandlers;
