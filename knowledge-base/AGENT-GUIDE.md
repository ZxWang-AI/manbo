...
> 当前仓库提供本地/受控验证用的私密案件与材料接口，但尚未获准接收真实生产数据；没有公开举报、地图或公司查询产品。Agent 不得把本地接口或设计能力说成已通过生产门禁。
...
8. **数据最小化**。不要求真实身份、IP 或设备指纹；方案 A 允许用户主动保存加密原始证据和必要对话，但必须经过安全提示、扫描、授权和删除控制。用户不愿回答时提供退出或跳过选项。
