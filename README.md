...
慢波 Manbo 是一个面向强迫劳动相关信息整理与举报准备的 AI-Native 项目。仓库包含本地可运行的应用基础、知识库和端到端测试；当前仅用于本地/受控验证，**尚未开放为可供公众提交真实案件的生产服务，也不提供执法服务或法律认定**。
...
| 应用基础（本地） | 账户/会话、私密案件、对话持久化、材料安全流水线、语音、管理员审核、用户删除/导出与 WCAG 回归已实现并通过本地测试；AI 质量门禁已独立接入 CI，覆盖危机优先、法律越界降级、未知来源拒绝和黄金案例；解析阶段具备输入/输出资源上限和超时回退保护；材料完成落库后会尝试本地异步处理，失败不会删除原件；真实部署仍需 Gate 1 证据 |
| 长期私密证据托管 | 方案 A 的本地/staging 加密适配器、分片上传、处理重试与安全摘要接口已实现；代码已加入可显式启用的 PostgreSQL 持久化处理队列、租约回收、长任务心跳续租、受限退避、死信状态和受监督 worker 入口，但默认 Web 路径仍为进程内适配器，且尚未完成真实对象存储/KMS、扫描/隔离解析、worker 监督、指标告警和备份恢复演练，因而不能开启生产托管 |
...
本项目提供信息整理与渠道导航，不构成法律意见或官方认定。用户自述和材料（如进入档案）均为未经独立核实的信息，不等于事实；当前不提供公开举报页。任何在线行为仍可能留下设备、网络或证据反推痕迹；平台不作“绝对匿名”或“绝对安全”承诺。若存在即时暴力、拘禁、自伤、未成年人或人口贩运风险，应优先联系当地紧急服务或专业机构。
...
慢波 Manbo 目前处于研究、知识库和安全应用基础建设阶段，目标是帮助劳动者和知情者安全地理解强迫劳动指标、整理自述、查找官方渠道，并保留自主决定权。账户、私密案件、对话持久化、受控材料接口和管理员审核已具备本地实现；真实证据托管、公开举报、地图与公司查询均未上线。平台不会替代执法、律师、心理咨询或紧急救援。
