...
| 应用基础（本地） | 账户/会话、私密案件、对话持久化、材料安全流水线、语音、管理员审核、用户删除/导出与 WCAG 回归已实现并通过本地测试；AI 质量门禁已独立接入 CI，覆盖危机优先、法律越界降级、未知来源拒绝和黄金案例；材料完成落库后会尝试本地异步处理，失败不会删除原件；真实部署仍需 Gate 1 证据 |
